    ...
    <div className="container" style={{ paddingTop: "2rem", paddingBottom: "3rem" }}>
      <div style={{ maxWidth: 760, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap", fontSize: "0.875rem", color: "var(--text-2, #6b7280)", marginBottom: "1.5rem", alignItems: "center" }}>
          ...
          {article.updated_at && article.updated_at !== article.published_at && (
            <span>Оновлено: <time dateTime={isoDate(article.updated_at)}>{formatDate(article.updated_at)}</time></span>
          )}
        ...
        {article.author_slug && (
          <div style={{ display: "flex", gap: "1.25rem", alignItems: "flex-start", padding: "1.5rem", border: "1px solid var(--border, #e5e7eb)", borderRadius: "12px", background: "var(--surface-2, #f9fafb)", marginBottom: "3rem" }}>
            ...
            <div>
              <div style={{ fontSize: "0.75rem", color: "var(--text-2, #6b7280)", marginBottom: "0.2rem" }}>Автор</div>
      ...
      {related.length > 0 && (
        <div style={{ marginTop: "1rem" }}>
          <h2 style={{ fontSize: "1.2rem", fontWeight: 600, marginBottom: "1.5rem" }}>Схожі статті</h2>
          <div className="grid">
            {related.map((a) => (
